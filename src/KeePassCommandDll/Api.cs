...
namespace KeePassCommandDll
{
    public static class Api
    {
        ...
        public static ApiSignRdpUsingBuildstampOnKeePassHostResponse signRdpUsingBuildstampOnKeePassHost(string KeePassEntryTitle, string filenameToCodeSign)
        {
            StringBuilder command = new StringBuilder("sign-rdp-using-buildstamp\t");
            command.Append(KeePassEntryTitle);
            command.Append('\t');

            if (!File.Exists(filenameToCodeSign))
                throw new Exception("sign-rdp-using-buildstamp: file \"" + filenameToCodeSign + "\" does not exist.");
            string filenameOnly = Path.GetFileName(filenameToCodeSign);
            ...
            ApiSignRdpUsingBuildstampOnKeePassHostResponse result = new ApiSignRdpUsingBuildstampOnKeePassHostResponse()
            {
                ExitCode = 99,
                StdOut = null,
                StdErr = null,
                            ...
                            switch (no)
                            {
                                ...
                                case 3: //filenameOnly signedBytes
                                    if (result.ExitCode == 0)
                                    {
                                        string responseFilenameOnly = Encoding.UTF8.GetString(Convert.FromBase64String(item.Parts[0]));
                                        if (responseFilenameOnly != filenameOnly)
                                            throw new Exception("sign-rdp-using-buildstamp response failure, expected row 3 \"" + filenameOnly + "\".");
                                        result.SignedBytes = Convert.FromBase64String(item.Parts[1]);
                                    }
                                    break;
                            }
