...
{
    public class CommandSignRdpUsingBuildstamp : ICommand
    {
        ...
        {
            Debug.OutputLine("Starting command sign-rdp-using-buildstamp");
            ...
            if (titles.Count == 1)
            {
                foreach(var keypair in titles)
                {
                    if (keypair.Value.Count == 1)
                    {
                        ...
                        {
                            SignRdpUsingEntry(Debug, KeePassHost, output, entry, filenameOnly, fileBytes);
                            break;
                        }
                    }
                    break;
            ...
            Debug.OutputLine("Ended command sign-rdp-using-buildstamp");
        }

        private void SignRdpUsingEntry(DebugLog Debug, IPluginHost KeePassHost, StringBuilder output, PwEntry entry, string filenameOnly, byte[] fileBytes)
        {
            ...
            try
            {
                ...
                List<string> buildstampArguments = new List<string>()
                {
                    ...
                    "--keepass-pkcs11-title", EntriesHelper.GetEntryField(Debug, KeePassHost, entry, PwDefs.TitleField),
